...
toc: true
date-format: long
abstract: "Municipal Licensing & Standards (ML&S) issues licences to various type of businesses and trades in Toronto city. This licence dataset is used for the analysis to access if there is any discrimination in issuing licences to various categories or not. The data set is analysed and compared for the period of 2015-2018 and 2019-2022. Evidently, certain categories have licence durations that differ greatly from one another. For example, categories were granted licences for longer periods of time during the 2015–2018 period than they were during the latter period."
format: pdf
...
When comparing these pattern, it is being considered that there are numerous factors, including both legislative and larger social-economic dynamics, 
...
The purpose of this paper is to identify if there are any significant changes or trends in the number of licences and permits issued in various categories during last two terms by municiple administrator. The remainder of this paper is structured as follows. @sec-data elaborate about
the data-set used for the analysis. @sec-result shows the result of the analysis. 
...
The data set used in this analysis is obtained from the City of Toronto’s Open-Data Toronto Library [@citeOpenData]. The data set is entitle with the name "Municipal Licensing and Standards - Business Licences and Permits" [@toronto_open_data]. Data was collected and analyzed with the utilization of the programming software [@citer]. Along with that the following packages are also used, ggplot2 for plotting [@citeGGPlot], tidyverse for data frame manipulation @citeTidyverse, knitr for pdf rendering [@citeKnitr] and kableExtra for styling tables [@citekableExtra]. 

The raw data set contains 32000 observations each having 18 features. However, for the analysis I only require a few of the features. The data set is cleaned by removing the unnecessary features. I also performed feature engineering to generate a couple of features. These will be discussed further in detail. A sample of cleaned data set is shown here @tbl-sampledata.
...
licenses issued during this period. The overall distribution is shown here @tbl-catall.
...
result <- kable(df2) |> kable_classic(full_width = FALSE)
result
```
...
result <- kable(df3) |> kable_classic(full_width = FALSE)
result
```
...
The analysis of average license duration across different categories in Toronto sheds light on intriguing trends and variations between two distinct time periods, namely before and after 2018. In the earlier period, specific categories stood out with notably extended average durations. Noteworthy examples include "Temporary Sign Provider," "Taxicab Broker," and "Private Transportation Company," indicating a prolonged adherence to licensing requirements within these sectors. These findings suggest a potential stability and sustained compliance within these businesses, aligning with the idea that certain industries may operate with consistent licensing practices over the years.
